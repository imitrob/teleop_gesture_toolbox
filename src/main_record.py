...
from promps.promp_lib import ProMPGenerator, CustomPlot
...
def main():
    ...
    while time.time()-t1 < 20:
        if ml.md.present():
            ...
            if gl.gd.l.static.relevant():
                ...
                #probs = gl.gd.l.static.relevant().probabilities
                #names = gl.gd.l.static.info.names
    ...
    #np.save(settings.paths.learn_path+"gesture_data.npy", gl.gd, allow_pickle=True, fix_imports=True)
    ...
    for n in range(2):#gl.gd.r.dynamic.info.n):
        viz.visualize_2d(list(zip(viztime_dyn, vizdata_dyn[n])),label=f"{gl.gd.r.dynamic.info.names[n]}", xlabel='time, leap seq [-] (~100 seq/sec)', ylabel='Gesture probability', start_stop_mark=False)

    for n in range(2):#gl.gd.r.static.info.n):
        viz.visualize_2d(list(zip(viztime_sta, vizdata_sta[n])),label=f"{gl.gd.r.static.info.names[n]}", xlabel='time, leap seq [-] (~100 seq/sec)', ylabel='Gesture probability', start_stop_mark=False)
