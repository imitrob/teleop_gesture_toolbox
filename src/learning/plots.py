...
def make_data_boundaries(data_test):
    ...
    Returns:
        data_test (2darray): [(time + 3*n)], where [mean_values, low_boundaries, high_boundaries]
    '''
...
7 static gestures, args: all_defined, take_every_10, 3000 iter.', 'Balanced Accuracy [%]', 'Amount train data from dataset [%]', ['test data', 'train data'],\
...
# Different seeds in mind

TITLE = 'Experiment 4.1, Grid search (Iterations) (Seed independent)'
ARGS = '7 static gestures, all_defined, take_every_10, 1s, interpolate, iter=<1000-4000>, 0.25split, 50n_hidden. tested on remaining data'
data_test = np.array([
[1000, 2000, 3000, 3500, 4000],
[91.5915915915916, 96.3963963963964, 97.2972972972973, 97.14714714714715,96.3963963963964],
[79.12912912912913,91.8918918918919, 91.44144144144144,90.69069069069069, 89.03903903903904],
[88.88888888888889,94.5945945945946, 94.5945945945946,94.29429429429429, 94.29429429429429],
[88.28828828828829,90.990990990991, 88.88888888888889,87.98798798798799, 89.33933933933935],
[80.78078078078079,87.68768768768768, 86.93693693693693,87.08708708708708, 85.73573573573574]
])

data_test = np.c_[ data_test[:,0:1], [1500, 94.8948948948949, 90.24024024024024, 92.49249249249249, 90.24024024024024, 85.88588588588588], data_test[:,1:]]
data_test = np.c_[ data_test[:,0:3], [2500, 97.14714714714715, 91.8918918918919, 94.5945945945946, 90.69069069069069, 86.93693693693693], data_test[:,3:]]
data_test = np.c_[ data_test[:,0:2], [1750, 95.1951951951952, 90.990990990991, 93.09309309309309, 91.44144144144144, 87.08708708708708], data_test[:,2:]]
data_test = np.c_[ data_test[:,0:4], [2250, 96.3963963963964, 91.74174174174175, 94.5945945945946, 90.69069069069069, 86.63663663663664], data_test[:,4:]]

data_train = np.array([
[1000, 2000, 3000, 3500, 4000],
[92.03406813627254, 95.79158316633266, 96.79358717434869,96.5430861723447 ,95.39078156312625],
[81.3627254509018, 91.48296593186373, 90.13026052104209,89.52905811623246, 88.22645290581163],
[91.38276553106212,95.89178356713427, 94.93987975951904,94.68937875751503,94.2384769539078],
[88.17635270541082,91.83366733466933,89.17835671342685,88.77755511022045,91.18236472945893],
[81.4128256513026,88.47695390781564, 88.02605210420842,87.2745490981964,87.2745490981964]
])
data_train = np.c_[ data_train[:,0:1], [1500, 94.63927855711422, 89.77955911823648, 94.98997995991984, 90.93186372745491, 86.57314629258516], data_train[:,1:]]
data_train = np.c_[ data_train[:,0:3], [2500, 96.29258517034069, 91.53306613226452, 95.79158316633266, 92.48496993987976, 87.7254509018036], data_train[:,3:]]
data_train = np.c_[ data_train[:,0:2], [1750, 95.24048096192385, 90.8817635270541, 95.44088176352705, 91.53306613226452, 87.97595190380761], data_train[:,2:]]
data_train = np.c_[ data_train[:,0:4], [2250, 95.94188376753507, 91.48296593186373, 95.44088176352705, 92.33466933867736, 88.07615230460922], data_train[:,4:]]
...
'''
Experiment 5.1, Grid search (Iterations) (Seed independent)
Parameters:
    all_defined, take_every_10, 1s, 0.25split, 10000samples, iter=<1000,6000>
Results:
    iterations, accuracy (test) [%], accuracy (train) [%]
'''
TITLE = 'Experiment 5.1, grid search (Iterations) (Seed independent)'
ARGS = '7 static gestures, all_defined, take_every_10, 1s, 0.25split, 10000samples, tested on remaining data'
...
# All the data (backup)
Accuracies = [[78.79464285714286, 86.60714285714286, 90.625, 86.83035714285714, 91.51785714285714, 94.86607142857143, 87.27678571428571, 96.42857142857143, 94.41964285714286], [83.25892857142857, 89.50892857142857, 89.0625, 88.61607142857143, 89.73214285714286, 85.71428571428571, 89.95535714285714, 90.84821428571429, 93.75], [79.01785714285714, 91.51785714285714, 87.27678571428571, 94.86607142857143, 91.51785714285714, 86.38392857142857, 88.61607142857143, 85.71428571428571, 85.26785714285714], [84.15178571428571, 88.83928571428571, 92.85714285714286, 90.84821428571429, 89.0625, 89.95535714285714, 94.86607142857143, 92.41071428571429, 89.0625], [84.375, 86.16071428571429, 91.51785714285714, 93.08035714285714, 89.95535714285714, 89.95535714285714, 91.29464285714286, 95.53571428571429, 95.75892857142857]]
Accuracies_train = [[78.15063385533185, 89.56002982848621, 91.34973900074571, 88.21774794929156, 92.46830723340791, 94.7054436987323, 91.20059656972408, 95.3765846383296, 95.07829977628636], [83.07233407904549, 91.64802386278896, 91.34973900074571, 91.2751677852349, 92.39373601789708, 85.83146905294556, 87.54660700969426, 94.92915734526473, 94.2580164056674], [79.19463087248322, 91.42431021625652, 87.69574944071589, 93.21401938851604, 93.51230425055928, 87.69574944071589, 92.54287844891871, 91.8717375093214, 85.68232662192393], [85.30947054436987, 87.32289336316182, 91.72259507829978, 91.64802386278896, 89.26174496644296, 90.1565995525727, 94.1834451901566, 92.46830723340791, 91.20059656972408], [83.51976137211037, 89.70917225950782, 89.41088739746458, 92.39373601789708, 90.23117076808353, 94.03430275913497, 91.20059656972408, 96.04772557792693, 95.3765846383296]]
...
'''
Experiment 5.2, grid search, with interpolate (Seed independent)
Parameters:
    all_defined, take_every_10, 1s, interpolate, 0.25split, 10000samples, iter=<1000,6000>, tested on remaining data
'''
TITLE = 'Experiment 5.2, Grid search (Iterations) with interpolate (Seed independent)'
ARGS = '7 static gestures, all_defined, take_every_10, interpolate, 1s, 0.25split, 10000samples, tested on remaining data'
...
plot(np.r_[ data_test, data_train[1:], test_previous[1:], train_previous[1:] ], TITLE+'\nArgs: '+ARGS, 'Iterations <1000,6000> [-]', 'Balanced Accuracy [%]', ['test data', 'train data', '/wo interp test', '/wo interp train'], minmaxboundaries4plot=True)
...
'''
Experiment 5.6, (Wrong) 90% trained, updated with 10% data, only 5 update iterations (Seed independent)
Parameters:
    all_defined, take_every_10, 1s, interpolate, 0.3split, 3000iter, tested on 30% data, 5iupdate
'''
TITLE = 'Experiment 5.6, (Wrong) 90% trained, updated with 10% data, only 5 update iterations (Seed independent)'
ARGS = '7 static gestures, all_defined, take_every_10, interpolate, 1s, 0.3split, 3000iter, tested on 30% data'

accuracies = [[90, 100 ]]
accuracies_train = [[ 90, 100 ]]
#Seed1
accuracies.append( [93.61702127659575, 15.894868585732166] )
accuracies_train.append( [93.67541766109785, 15.572792362768498] )
#Seed2
accuracies.append( [93.86733416770964, 15.018773466833544] )
accuracies_train.append( [95.34606205250597, 15.214797136038186] )
#Seed3
accuracies.append( [87.35919899874844, 8.260325406758447] )
accuracies_train.append( [87.94749403341288, 10.501193317422434] )
#Seed4
accuracies.append( [94.74342928660826, 20.90112640801001] )
accuracies_train.append( [96.06205250596659, 19.391408114558473] )
#Seed5
accuracies.append( [93.4918648310388, 16.270337922403] )
accuracies_train.append( [93.4964200477327, 16.2291169451074] )
...
TITLE = 'Experiment 5.6.1, (Right) 90% trained, updated with 10% data, only 5 update iterations (Seed independent)'
...
TITLE = 'Experiment 5.6.2, (Right) 90% trained, updated with 10% data, 2500 update iterations (Reason decrease is overtraining with new data) (Seed independent)'
ARGS = 'all_defined, take_every_10, 1s, 0.25split, 2500iter, tested on 25% data, 2500iupdate'
...
'''
Experiment 5.8, (Right) Sample records for every gesture k=[1,2,5,10,20,50] (Seed independent)
Parameters:
    all_defined, take_every_10, 1s, interpolate, 0.3split, 3000iter, tested on 30% data, 2000iupdate
'''
TITLE = 'Experiment 5.8, (Right) Sample records for every gesture k=[1,2,5,10,20] (Seed independent)'
...
TITLE = 'Experiment 5.8.1, Sample records for every gesture k=[1,2,5,10,20] (Seed independent)'
...
TITLE = 'Experiment 5.9, (Right) 17% initial data 17% update (Seed independent)'
ARGS = '7 static gestures, all_defined, take_every_10, 1s, 0.3split, 3000iter, tested on 30% data, 3000iupdate'
...
TITLE = 'Experiment 5.9.1, (Right) 17% initial data 17% update (Seed independent)'
ARGS = '7 static gestures, all_defined, take_every_10, 1s, interpolate, 0.3split, 3000iter, tested on 30% data, 3000iupdate'
accuracies = [[ 17.5, 35, 52.5, 70 ]]
accuracies_train = [[ 17.5, 35, 52.5, 70 ]]
...
'''
Experiment 6.1, Take every 3 time-samples (Seed independent)
Parameters:
    all_defined, take_every_3, 1s, 0.25split, <1000-30000>iter, tested on 25% data
'''
TITLE = 'Experiment 6.1, Take every 3 time-samples (Seed independent)'
ARGS = '7 static gestures, all_defined, take_every_3, 1s, 0.25split, <1000-30000>iter, tested on 25% data'
