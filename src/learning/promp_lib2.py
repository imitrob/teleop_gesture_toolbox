...
def construct_promp_trajectories(Xpalm, DXpalm, Y):
    ...
    assert isinstance(DXpalm, np.ndarray), "Not right type"
...
def my_promp(data, out='w'):
    ...
    d_promp.train()
    if out == 'w':
        return np.diag(d_promp._sigma_W)
    elif out == 'm':
        return d_promp._mean_W
    elif out == 's':
        return d_promp._sigma_W
