...
class DiscretePROMP(object):
    ...
    def generate_basis_function(self, phase_z, phase_zd, phase_zdd):
        ...
        # basis functions
        phase_minus_centre = np.array(list(map(lambda x: x - self._bfs_centres,np.tile(phase_z, (self._n_bfs, 1)).T))).T
    ...
    def generate_trajectory(self, phase_speed=1., randomness=1e-4):
        ...
        #print("new_mean_W",np.array(new_mean_W).shape)
        #print("new_sigma_W",np.array(new_sigma_W).shape)
